...
public class ArrayWorker {
	...
	public static Entity[] resize(Entity[] pEntities) {
		return Arrays.copyOf(pEntities, pEntities.length * 2);
	}
	...
	public static Entity[] addToArray(Entity pEntity, Entity[] pEntities) {
		int position = getFreePosition(pEntities);
		if(position<0) {
			pEntities = resize(pEntities);
			position = getFreePosition(pEntities);
		}
